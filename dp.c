...
void test(int i)
{
	...
	*/
	...
	{
		//Particular philosopher's left and right is not eating, on top of that he is hungry. Make his state eating
		situation[i]=EATING;
...
void dp_init(int N)
{
	//Basic control statement whether user entered a in-range number
	if(N<5 || N>20)
	{
		perror("You have entered out-of-limit number."); 
	     return;
	}
	//Initialization of # of philosophers.
	philosopherCount=N;
	int i=0;
	//Initialization of condition variables for each philosopher.
	for(i=0;i<N;i++)
	{
